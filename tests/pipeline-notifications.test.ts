...
test('Creates execution state change rule', () => {
  ...
  const notifications = new PipelineNotifications(stack, 'TestPipelineNotifications', { pipeline, receivers: 'testreceivers'});
...
test('Allows customizing the message', () => {
  const stack = new Stack();
  const pipeline = fakePipeline(stack);
  const notifications = new PipelineNotifications(stack, 'TestPipelineNotifications', { pipeline, receivers: 'testreceivers', messageText: 'My message for test pipeline.'});
  expect(stack).to(haveResourceLike('AWS::Events::Rule', {
    "Targets": [
      {
        "Arn": {
